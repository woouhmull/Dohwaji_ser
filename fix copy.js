...
app.post('/user/login', async (req, res) => {
    ...
    if(data!=null){
      ...
      if(data.password ==req.body.password && data.userName == req.body.userName){
        //정보 완벽!
        auth=1;
      }
      else{
...
app.post('/user/register', async (req, res) => {
    let collection = myDB.db('ye').collection('yelection')
    flag = collection.findOne({$and: [ {"name": {$eq:req.body.name}},{"number":20}]});
...
app.post('/user/answer', async (req, res) => {
    ...
    s=req.body.answer
    var target=collection.findOne({$and: [ {"name": {$eq:req.body.name}},{"number":{$eq:req.body.number}}]})
    //collection 으로 바꿔보기
    if(target ==null)
    {
    ...
    {
        collection.updateOne({target._id},{req.body})
    }
    var flag =0
    ...
    if(req.body.number >= 1 && req.body.number <= 5)
    {
      ...
      process1.stdout.on('data', async data1 => {
      console.log(data1.toString()[1]);
      var ID = req.body._id
      await collection.updateOne({_id: ID},{$set: {"result1" :parseInt(data1.toString()[1])}});

    });
      process2.stdout.on('data',async data2 => {
      console.log(data2.toString()[1]);
      var ID = req.body._id
      await collection.updateOne({_id: ID},{$set: {"result2" :parseInt(data2.toString()[1])}});
    });
    ...
    else if(req.body.number >= 16 && req.body.number <= 19)
    {
      ...
      console.log(data2.toString()[1]);
      var ID = req.body._id
      await collection.updateOne({_id: ID},{$set: {"result2" :parseInt(data2.toString()[1]*1)}});
      });

    }
    else if(req.body.number ==20 )
    {
      const process1 = spawn('python', ['./Conscientiousness.py', s]);
      const process2 = spawn('python', ['./YesNo.py', s]);
      console.log(req.body)
...
app.post('/user/result', async (req, res) => {
  let collection = myDB.db('ye').collection('answer')
//데이터를 찾아온다 이메일과 동일한 부분을
  //const check1 = await collection.findOne({name: {$eq:req.body.userName}})
  var re_E=0;
  var re_A=0;
  var re_O=0;
  var re_C=0;
